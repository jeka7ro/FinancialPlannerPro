...
export class ImportExportService {
    ...
    async importCompaniesFromExcel(buffer) {
        try {
            const workbook = XLSX.read(buffer, { type: 'buffer' });
            const worksheet = workbook.Sheets[workbook.SheetNames[0]];
            const data = XLSX.utils.sheet_to_json(worksheet);
    ...
    async importLocationsFromExcel(buffer) {
        try {
            const workbook = XLSX.read(buffer, { type: 'buffer' });
            const worksheet = workbook.Sheets[workbook.SheetNames[0]];
            const data = XLSX.utils.sheet_to_json(worksheet);
    ...
    async importCabinetsFromExcel(buffer) {
        try {
            const workbook = XLSX.read(buffer, { type: 'buffer' });
            const worksheet = workbook.Sheets[workbook.SheetNames[0]];
            const data = XLSX.utils.sheet_to_json(worksheet);
    ...
    async exportLegalDocumentsToExcel() {
    ...
    async exportOnjnReportsToExcel() {
    ...
    async exportRentAgreementsToExcel() {
    ...
    async importFromExcel(module, buffer) {
        switch (module) {
            case "companies":
    ...
    async exportToExcel(moduleName) {
        switch (moduleName) {
            case 'users':
    ...
    async importProvidersFromExcel(buffer) {
