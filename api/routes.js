...
import { insertCompanySchema, insertLocationSchema, insertProviderSchema, insertCabinetSchema, insertGameMixSchema, insertSlotSchema, insertInvoiceSchema, insertRentAgreementSchema, insertLegalDocumentSchema, insertOnjnReportSchema } from "../shared/schema.js";
...
export async function registerRoutes(app) {
        ...
        try {
            ...
            if (typeof preprocessedData.locationIds === 'string') {
                if (preprocessedData.locationIds === '' || preprocessedData.locationIds === null || preprocessedData.locationIds === undefined) {
                    ...
                    preprocessedData.locationIds = preprocessedData.locationIds.split(',').map((id) => parseInt(id.trim())).filter((id) => !isNaN(id));
                }
            }
    ...
    app.post("/api/:module/import", requireAuth, upload.single('file'), async (req, res) => {
        try {
            if (!req.file) {
    ...
    app.get("/api/:module/template", requireAuth, async (req, res) => {
        try {
            const module = req.params.module;
    ...
    app.post("/api/:entityType/:entityId/attachments", requireAuth, uploadAttachment.single('file'), async (req, res) => {
        try {
            if (!req.file) {
                return res.status(400).json({ message: "No file uploaded" });
            }
            ...
            const userId = req.session.userId;
            const attachment = await fileService.saveFile(req.file, entityType, parseInt(entityId), userId, description);
            res.json(attachment);
        }
        catch (error) {
    ...
    app.get("/api/:entityType/:entityId/attachments", requireAuth, async (req, res) => {
        try {
            const { entityType, entityId } = req.params;
    ...
    app.get("/api/import-tutorial", requireAuth, async (req, res) => {
        try {
